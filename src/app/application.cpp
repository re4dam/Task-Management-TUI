...
bool Application::contains_substring(const std::string& str, const std::string& sub) {
...
std::string Application::trim(const std::string& str) {
...
int Application::levenshtein_distance(const std::string& s1, const std::string& s2) {
    int len1 = s1.length();
    int len2 = s2.length();
...
void Application::execute_command(const std::string& cmd_query, const std::vector<size_t>& pending_indices, const std::vector<size_t>& completed_indices) {
    std::string cmd = cmd_query;
    if (!cmd.empty() && cmd[0] == ':') {
        cmd = cmd.substr(1);
    }
    cmd = trim(cmd);
    ...
    if (cmd_name == "q" || cmd_name == "quit") {
        ...
        if (cmd_args.empty()) {
            ...
            ThemeColors theme;
            if (persistence.load_theme(cmd_args, theme)) {
                Tui::apply_theme(theme);
                active_theme = cmd_args;
                persistence.save_config(keymap, active_theme);
                clear();
            } else {
                Tui::show_confirm_dialog("Error", "Could not load theme: " + cmd_args);
            }
        }
    } else {
...
void Application::setup_windows() {
    if (header_win) delwin(header_win);
    if (lists_win) delwin(lists_win);
    if (tasks_win) delwin(tasks_win);
    if (details_win) delwin(details_win);
...
int Application::run() {
    ...
    persistence.load_config(keymap, active_theme);
    persistence.generate_sample_theme_if_missing();
    ThemeColors theme;
    persistence.load_theme(active_theme, theme);
    Tui::apply_theme(theme);
    
    lists = persistence.load_tasks();
    if (lists.empty()) {
        List default_list;
        default_list.name = "Inbox";
        lists.push_back(default_list);
    ...
    while (running) {
        ...
        if (!lists.empty() && selected_list_idx < lists.size()) {
            ...
            for (size_t i = 0; i < active_list.tasks.size(); ++i) {
                const auto& t = active_list.tasks[i];
                if (!search_query.empty() && !contains_substring(t.title, search_query)) {
                    continue;
                }
                if (t.completed) {
        ...
        Tui::draw_lists(lists_win, lists, selected_list_idx, active_focus);
        Tui::draw_tasks(tasks_win, lists, selected_list_idx, selected_task_idx, active_focus, pending_indices, completed_indices, keymap);
        ...
        if (!lists.empty() && selected_list_idx < lists.size()) {
            ...
            if (selected_task_idx < pending_indices.size() + completed_indices.size()) {
                size_t actual_task_idx = (selected_task_idx < pending_indices.size())
                                         ? pending_indices[selected_task_idx]
                                         : completed_indices[selected_task_idx - pending_indices.size()];
                current_task = active_list.tasks[actual_task_idx];
                has_selection = true;
            }
        }
        Tui::draw_details(details_win, has_selection, current_task, details_scroll_idx, active_focus);
        ...
        if (active_mode == Mode::Command) {
            ...
            wmove(footer_win, 1, offset + 8 + search_query.length());
            wnoutrefresh(footer_win);
        } else {
            curs_set(0);
        }
        ...
        if (active_mode == Mode::Command) {
            if (ch == 27) {
                ...
                execute_command(command_query, pending_indices, completed_indices);
                active_mode = Mode::Normal;
                command_query.clear();
            } else if (ch == '\t') {
                ...
                if (action == Action::MoveUp) {
                    ...
                    if (!lists.empty() && selected_list_idx < lists.size()) {
                        ...
                        if (selected_task_idx < pending_indices.size() + completed_indices.size()) {
                            size_t actual_task_idx = (selected_task_idx < pending_indices.size())
                                                     ? pending_indices[selected_task_idx]
                                                     : completed_indices[selected_task_idx - pending_indices.size()];
                            auto& t = active_list.tasks[actual_task_idx];
                            if (!t.completed) {
                                t.completed = true;
                                if (t.recurrence != RecurrenceRule::None) {
                                    Task next_task = t;
                                    next_task.completed = false;
                                    if (!t.time_value.empty()) {
                                        bool success = false;
                                        std::tm tm = string_to_tm(t.time_value, success);
                                        if (success) {
                                            if (t.recurrence == RecurrenceRule::Daily) {
                                                tm.tm_mday += 1;
                                            } else if (t.recurrence == RecurrenceRule::Weekly) {
                                                tm.tm_mday += 7;
                                            } else if (t.recurrence == RecurrenceRule::Monthly) {
                                                tm.tm_mon += 1;
                                            }
                                            std::mktime(&tm);
                    ...
                    if (!lists.empty() && selected_list_idx < lists.size()) {
                        ...
                        if (!active_list.tasks.empty() && selected_task_idx < pending_indices.size() + completed_indices.size()) {
                            ...
                            std::string title, desc, time_val;
                            RecurrenceRule recurrence = RecurrenceRule::None;
                            if (Tui::show_task_edit_dialog(t.type, t.title, t.description, t.time_value, t.recurrence, title, desc, time_val, recurrence, footer_win, &active_mode)) {
                                t.title = title;
                                t.description = desc;
                                t.time_value = time_val;
